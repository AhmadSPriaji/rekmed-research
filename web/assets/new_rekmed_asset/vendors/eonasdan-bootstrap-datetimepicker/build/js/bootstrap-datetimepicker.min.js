...
!(function (a) {
  ...
  var c = function (c, d) {
    var e,
          ...
          d.sideBySide && z() && y()
            ? (b.addClass("timepicker-sbs"),
              b.append(
                a("<div>")
                  .addClass("row")
                  .append(c.addClass("col-sm-6"))
                  .append(e.addClass("col-sm-6"))
              ),
              b.append(h),
              b)
            : ("top" === d.toolbarPlacement && g.append(h),
              z() &&
                g.append(
                  a("<li>")
                    .addClass(d.collapse && y() ? "collapse in" : "")
                    .append(c)
                ),
              ...
              y() &&
                g.append(
                  a("<li>")
                    .addClass(d.collapse && z() ? "collapse" : "")
                    .append(e)
                ),
      ...
      I = function (a) {
        o &&
          (a && (i = Math.max(p, Math.min(2, i + a))),
          o
            .find(".datepicker > div")
            .hide()
            .filter(".datepicker-" + q[i].clsName)
            .show());
      },
      J = function () {
        var b = a("<tr>"),
      ...
      M = function (a, b) {
        return a.isValid()
          ? d.disabledDates && K(a) && "M" !== b
            ? !1
            : d.enabledDates && !L(a) && "M" !== b
            ? !1
            : d.minDate && a.isBefore(d.minDate, b)
            ? !1
            : d.maxDate && a.isAfter(d.maxDate, b)
            ? !1
            : "d" === b && -1 !== d.daysOfWeekDisabled.indexOf(a.day())
            ? !1
            : !0
          : !1;
      },
      N = function () {
        for (
          var b = [], c = l.clone().startOf("y").hour(12);
          c.isSame(l, "y");
      ...
      O = function () {
        ...
        b.find(".disabled").removeClass("disabled"),
          M(l.clone().subtract(1, "y"), "y") || c.eq(0).addClass("disabled"),
          c.eq(1).text(l.year()),
          M(l.clone().add(1, "y"), "y") || c.eq(2).addClass("disabled"),
          d.removeClass("active"),
          k.isSame(l, "y") && d.eq(k.month()).addClass("active"),
          d.each(function (b) {
      ...
      Q = function () {
        ...
        if (z()) {
          for (
            g.find(".disabled").removeClass("disabled"),
              h.eq(1).text(l.format(d.dayViewHeaderFormat)),
              M(l.clone().subtract(1, "M"), "M") ||
                h.eq(0).addClass("disabled"),
              M(l.clone().add(1, "M"), "M") || h.eq(2).addClass("disabled"),
              c = l.clone().startOf("M").startOf("week");
            !l.clone().endOf("M").endOf("w").isBefore(c, "d");

          )
            0 === c.weekday() &&
              ((e = a("<tr>")),
              d.calendarWeeks &&
                e.append('<td class="cw">' + c.week() + "</td>"),
              i.push(e)),
              (f = ""),
              ...
              e.append(
                '<td data-action="selectDay" class="day' +
                  f +
                  '">' +
                  c.date() +
                  "</td>"
      ...
      R = function () {
        var b = o.find(".timepicker-hours table"),
          ...
          e = a("<tr>");
        for (
          l.hour() > 11 && !f && c.hour(12);
          c.isSame(l, "d") &&
          (f || (l.hour() < 12 && c.hour() < 12) || l.hour() > 11);
      ...
      S = function () {
        for (
          var b = o.find(".timepicker-minutes table"),
            ...
            e = [],
            f = a("<tr>"),
            g = 1 === d.stepping ? 5 : d.stepping;
          l.isSame(c, "h");

        )
          c.minute() % (4 * g) === 0 && ((f = a("<tr>")), e.push(f)),
            f.append(
      ...
      U = function () {
        ...
        f ||
          o.find(".timepicker [data-action=togglePeriod]").text(k.format("A")),
          a
            .filter("[data-time-component=hours]")
            .text(k.format(f ? "HH" : "hh")),
          a.filter("[data-time-component=minutes]").text(k.format("mm")),
          a.filter("[data-time-component=seconds]").text(k.format("ss")),
          R(),
          S(),
          T();
      },
          ...
          ? ((a = a.clone().locale(d.locale)),
            ...
            void (M(a)
              ? ((k = a),
                ...
                V(),
                (m = !1),
                H({ type: "dp.change", date: k.clone(), oldDate: b }))
              : (d.keepInvalid || e.val(m ? "" : k.format(g)),
                H({ type: "dp.error", date: a }))))
          : ((m = !0),
            e.val(""),
            c.data("date", ""),
            H({ type: "dp.change", date: null, oldDate: b }),
            void V());
      },
      X = function () {
        var b = !1;
      ...
      Z = {
        ...
        selectMonth: function (b) {
          ...
          l.month(c),
            i === p
              ? (W(k.clone().year(l.year()).month(l.month())), d.inline || X())
              : (I(-1), Q());
        },
        selectYear: function (b) {
          ...
          l.year(c),
            i === p
              ? (W(k.clone().year(l.year())), d.inline || X())
              : (I(-1), Q());
        },
        selectDay: function (b) {
          ...
          a(b.target).is(".old") && c.subtract(1, "M"),
            ...
            W(c.date(parseInt(a(b.target).text(), 10))),
            y() || d.keepOpen || d.inline || X();
        },
        ...
        selectMinute: function (b) {
          W(k.clone().minutes(parseInt(a(b.target).text(), 10))),
            Z.showPicker.call(j);
        },
        selectSecond: function (b) {
          W(k.clone().seconds(parseInt(a(b.target).text(), 10))),
            Z.showPicker.call(j);
        },
        clear: Y,
        today: function () {
      ...
      $ = function (b) {
        return a(b.currentTarget).is(".disabled")
          ? !1
          : (Z[a(b.currentTarget).data("action")].apply(j, arguments), !1);
      },
      _ = function () {
        var c,
          f = {
        ...
        return e.prop("disabled") ||
          (!d.ignoreReadonly && e.prop("readonly")) ||
          o
          ? j
          : (d.useCurrent &&
              m &&
              ((e.is("input") && 0 === e.val().trim().length) || d.inline) &&
              ((c = b()),
      ...
      bb = function (a) {
        return (
          (a =
            b.isMoment(a) || a instanceof Date ? b(a) : b(a, h, d.useStrict)),
          a.locale(d.locale),
          a
        );
      },
      cb = function (a) {
        ...
        for (b in w)
          w.hasOwnProperty(b) &&
            w[b] === l &&
            (h.push(b), parseInt(b, 10) !== k && (i[b] = !0));
        for (b in d.keyBinds)
          if (
      ...
      fb = function () {
        e.on({ change: eb, blur: d.debug ? "" : X, keydown: cb, keyup: db }),
          c.is("input")
            ? e.on({ focus: _ })
            : n && (n.on("click", ab), n.on("mousedown", !1));
      },
      gb = function () {
        e.off({ change: eb, blur: X, keydown: cb, keyup: db }),
          c.is("input")
            ? e.off({ focus: _ })
      ...
      ib = function () {
        ...
        (g = a.replace(
          ...
          (f = g.toLowerCase().indexOf("a") < 1 && g.indexOf("h") < 1),
          x("y") && (p = 2),
          x("M") && (p = 1),
          x("d") && (p = 0),
          (i = Math.max(p, i)),
    ...
    if (
      ...
      (j.maxDate = function (a) {
        ...
        if (d.minDate && c.isBefore(d.minDate))
          throw new TypeError(
            "maxDate() date parameter is before options.minDate: " + c.format(g)
          );
        return (
          (d.maxDate = c),
          d.maxDate.isBefore(a) && W(d.maxDate),
          l.isAfter(c) && (l = c.clone()),
          V(),
          j
        );
      }),
      (j.minDate = function (a) {
        ...
        if (d.maxDate && c.isAfter(d.maxDate))
          throw new TypeError(
            "minDate() date parameter is after options.maxDate: " + c.format(g)
          );
        return (
          (d.minDate = c),
          d.minDate.isAfter(a) && W(d.minDate),
          l.isBefore(c) && (l = c.clone()),
          V(),
          j
        );
      }),
    ...
    return (
      ...
      e.is("input") && 0 !== e.val().trim().length
        ? W(bb(e.val().trim()))
        : d.defaultDate && void 0 === e.attr("placeholder") && W(d.defaultDate),
      d.inline && _(),
      j
    );
  };
      ...
      keyBinds: {
        ...
        left: function (a) {
          if (a) {
            var c = this.date() || b();
            a.find(".datepicker").is(":visible") &&
              this.date(c.clone().subtract(1, "d"));
          }
        },
        right: function (a) {
          if (a) {
        ...
        pageUp: function (a) {
          if (a) {
            var c = this.date() || b();
            a.find(".datepicker").is(":visible") &&
              this.date(c.clone().subtract(1, "M"));
          }
        },
        pageDown: function (a) {
          if (a) {
